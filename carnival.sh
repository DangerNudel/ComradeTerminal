...
# Spinning Soviet star locked at top of terminal + per-character rainbow blink prompt
...
__cv_init() {
    ...
    # Restore on shell exit OR signals.
    trap __cv_cleanup EXIT INT TERM HUP

    # Welcome banner.
    printf '%s%s ☭ welcome, comrade %s\n' \
        "${__CV_CSI}1;33m" "${__CV_BLINK}" "${__CV_RESET}"
    printf '%scommands:%s  %srb <cmd>%s = rainbow output    %sexit%s = leave    %sCtrl-C%s = leave\n' \
